...
public interface MeasuramentRepository extends JpaRepository<Measurament, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "SELECT SELECT CONCAT(WEEK(create_time) - WEEK(DATE_SUB(create_time, INTERVAL DAYOFMONTH(create_time) - 1 DAY ))+1, '/' ,MONTH(create_time),' ', YEAR(create_time)), ROUND(AVG(temperature),2) FROM measurament WHERE aquarium_id = '15425' AND create_time BETWEEN '2020-05-04 20:18:04' AND '2020-05-06 20:18:04' GROUP BY WEEK(create_time)", nativeQuery = true)
